...
x-skill is a CLI tool for the open agent skills ecosystem. It manages skill packages for 42+ AI coding agents (Claude Code, Cursor, Codex, Gemini CLI, etc.). Skills are defined in `SKILL.md` files with YAML frontmatter and can be installed from GitHub, GitLab, local directories, or well-known URLs.
...
- **Universal Agents**: Agents sharing `.agents/skills/` path (Claude Code, Cursor, Codex, etc.). Files written once, no symlinks needed.
